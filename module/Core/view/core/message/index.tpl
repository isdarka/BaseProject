<fieldset>
	<legend>{$i18n->translate('Message')} 
		{if 'core\controller\message::create'|isAllowed}
			<a href="{$baseUrl}/core/message/create" class="btn btn-success pull-right">{$i18n->translate('New Message')}</a>
		{/if}
	</legend>
	<div class="table-responsive">
	<form action="{$baseUrl}/core/message/index" method="get">
		<table class="table table-bordered table-condensed table-hover">
			<thead>
				<tr class="well">
					<th><input type="text" class="form-control" name="id_message" placeholder="{$i18n->translate('ID_MESSAGE')}" value="{$queryParams['id_message']}"></th>
					<th>
				...
				<tr class="well">
					<th>{$i18n->translate('IdMessage')}</th>
