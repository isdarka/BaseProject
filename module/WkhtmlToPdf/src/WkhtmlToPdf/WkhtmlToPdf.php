...
use WkhtmlToPdf\Exception\WkhtmlToPdfException;
/**
...
class WkhtmlToPdf
{
	...
	*/
	...
	{
		$this->_path = __DIR__ . "/data/";
		$this->_bin = __DIR__ . "/bin/wkhtmltopdf-i386";
...
// 			throw new WkhtmlToPdfException("Path to directory where to store files is not set");
// 		}

		
		if (!is_writable($this->_path))
		{
			throw new WkhtmlToPdfException("Path to directory where to store files is not writable");
		}
...
// 		$this->setPath($options['path']);
	...
	public function setPath($path)
	{
		if (realpath($path) === false) {
			throw new WkhtmlToPdfException("Path must be absolute");
		}
	...
	protected function _render()
	{
		if (mb_strlen($this->_html, 'utf-8') === 0 && empty($this->_url)) {
			throw new WkhtmlToPdfException("HTML content or source URL not set");
		}
		...
		if (strpos(mb_strtolower($content['stderr']), 'error')) {
			throw new WkhtmlToPdfException("System error <pre>" . $content['stderr'] . "</pre>");
		}

		if (mb_strlen($content['stdout'], 'utf-8') === 0) {
			throw new WkhtmlToPdfException("WKHTMLTOPDF didn't return any data");
		}

		if ((int)$content['return'] > 1) {
			throw new WkhtmlToPdfException("Shell error, return code: " . (int)$content['return']);
		}
		...
		switch ($mode) {
			case self::MODE_DOWNLOAD:
				if (!headers_sent()) {
					...
					throw new WkhtmlToPdfException("Headers already sent");
				}
				break;
			...
			case self::MODE_SAVE:
				...
				break;
			default:
				throw new WkhtmlToPdfException("Mode: " . $mode . " is not supported");
		}
	}
}